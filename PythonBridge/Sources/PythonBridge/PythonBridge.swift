...
// This interopability was witten by the Swift Community and altered by 
// AgnosticDev for the purposes of this post.
//
//===----------------------------------------------------------------------===//
//
...
public struct PyVal {
  ...
  subscript(dynamicMember member: String) -> PyVal {
    ...
    func dynamicallyCall(withKeywordArguments args: KeyValuePairs<String, Int>) -> PyVal {
        let result = PyObject_GetAttrString(borrowedPyObject, "\(args.first?.value ?? 0)")!
        return PyVal(owned: result)
    }
    
//    func dynamicallyCall(withKeywordArguments args: KeyValuePairs<String, [Int]>) {
//       PyObject_GetAttrString(borrowedPyObject, "\(args.first?.value ?? [])")
//    }
}
...
extension PyVal : CustomStringConvertible {
  public var description: String {
    ...
    if let stringDescription = String(Python.str.call(args: self)) {
...
public struct CheckingPyVal {
  ...
  public func get(member: String) -> PyVal? {
    ...
    guard let result = PyObject_GetAttrString(selfObj, member) else {
      PyErr_Clear()
      print("The member you were requesting was not found.  Returning nil.")
      return nil
    }
...
public struct PythonInterface {
  ...
  init() {
    ...
    public func attemptImport(_ name: String) throws -> PyVal {
...
// PyVal extension for converting from PyVal back to a Swift type easily.
extension PyVal {

  public func asInteger() -> Int {
    if let pyValInteger = Int(self) {
      return pyValInteger
    } else {
      return -1
    }
  ...
  // Try casting a PyVal to a Double
  public func asDouble() -> Double{
    if let pyValDouble = Double(self) {
      return pyValDouble
    } else {
      return -1.0
    }
  ...
  // Try casting a PyVal to a String
  public func asString() -> String {
    if let pyValString = String(self) {
      return pyValString
    } else {
      return ""
    }
